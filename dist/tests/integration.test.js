...
const behavioral_1 = require("../src/behavioral");
function withToken(guard, options = {}) {
...
function linearEvents(count, dx = 1, dy = 1, dt = 16) {
    const events = [];
...
function teleportEvents(count = 20) {
    const events = [];
    for (let i = 0; i < count; i++) {
        const t = i * 16;
...
function agentCadenceEvents(steps = 3) {
    const events = [];
    let t = 0;
    let p = 0;
    let x = 100, y = 100;
...
describe('TraceGuardAI — Protocol Layer', () => {
    const guard = new core_1.TraceGuardAI();
...
describe('BehavioralAnalyzer — Unit Tests', () => {
    const analyzer = new behavioral_1.BehavioralAnalyzer();
    // --- calculatePathLength ---
